...
from segmenter.gcn.model import GCNSegmenterModel
from segmenter.gcn import GCNSegmenter
from util.eval import edu_eval, gen_edu_report
from util.ltp import LTPParser

logger = logging.getLogger("train gcn segmenter")


def preprocessing(cdtb):
    with LTPParser() as parser:
        deps = {}
        logger.info("add dependency information to sentence")
        for paragraph in tqdm.tqdm(chain(*chain(cdtb.train, cdtb.validate, cdtb.test)), desc="parsing", unit=" sentence"):
            for sentence in paragraph.sentences():
                if sentence.sid not in deps:
                    dep = parser.parse(sentence.words)
                    deps[sentence.sid] = dep
                else:
                    dep = deps[sentence.sid]
                setattr(sentence, "dependency", dep)
...
def numericalize(instances, tags, word_vocab, pos_vocab, gcn_vocab, tag_label):
    trainset = []
    for (words, poses, graph), tags in zip(instances, tags):
        word_ids = [word_vocab[word] for word in words]
        pos_ids = [pos_vocab[pos] for pos in poses]
        tag_ids = [tag_label[tag] for tag in tags]
        graph_ids = [(x, y, gcn_vocab[l]) for x, y, l in graph]
        trainset.append((word_ids, pos_ids, graph_ids, tag_ids))
    return trainset


def gen_batch_iter(trainset, batch_size, num_gcn_label, use_gpu=False):
    ...
    while offset < num_instances:
        ...
        graph_inputs = np.zeros([num_batch, max_seqlen, max_seqlen, num_gcn_label], np.uint8)
        ...
        if use_gpu:
            ...
            graph_inputs = graph_inputs.cuda()
            masks = masks.cuda()
        yield (word_inputs, pos_inputs, graph_inputs, masks), tag_outputs
...
def main(args):
    ...
    # builkd
    preprocessing(cdtb)
    word_vocab, pos_vocab, gcn_vocab = build_vocab(cdtb.train)
    ...
    for nepoch in range(1, args.epoch+1):
        batch_iter = gen_batch_iter(trainset, args.batch_size, len(gcn_vocab), use_gpu=args.use_gpu)
