...
def main():
    ...
    for gold, parse, session in zip(golds, parses, parse_sessions):
        ...
        # score [steps, splits]
        fig, ax = plt.subplots()
        row_labels = ["step_%d" % i for i in range(1, session.scores.shape[0]+1)]
        col_labels = ["split_%d" % i for i in range(1, session.scores.shape[1]+1)]
        im, cbar = heatmap(session.scores, row_labels, col_labels, ax=ax,
                           cmap="YlGn", cbarlabel="split score")
